  ...
  <div class="m-4 p-4 bg-white shadow rounded">
    ...
    <form @submit.prevent="onSubmit">
      <div class="flex items-center justify-center flex-col w-full h-full mt-8">
        <!-- v-bind:class に {適用したいclass: 条件} classを動的に切り変えることができる -->
        <div
...
<script>
  ...
  export default {
    // 画面の状態を保存しておく場所 dataの中身の状態に変更があると画面の表示もdataに合わせて変更される
    data() {
      return {
        form: {
          name: {
    ...
    methods: {
      selectImage() {
        // ref="属性名"を指定したタグにthis.$refs.属性名で指定してDOM要素にアクセスできる
      ...
      async onSubmit() {
        ...
        try {
          ...
          console.log(e)
          const message = '登録に失敗しました'
          this.$store.dispatch('alert/set', message)
        }
      }
    }
  }
